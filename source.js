...
function initTextOverlay() { if (window._textOverlayReady) return; var glCanvas = document.getElementById('canvas'); var container = document.createElement('div'); container.style.position = 'relative'; container.style.display = 'inline-block'; container.style.width = glCanvas.style.width || (glCanvas.width + 'px'); container.style.height = glCanvas.style.height || (glCanvas.height + 'px'); if (glCanvas.style.width === '100%') { container.style.width = '100%'; container.style.height = '100%'; } glCanvas.parentNode.insertBefore(container, glCanvas); container.appendChild(glCanvas); var overlay = document.createElement('canvas'); overlay.id = 'textOverlay'; overlay.style.position = 'absolute'; overlay.style.left = '0'; overlay.style.top = '0'; overlay.style.width = '100%'; overlay.style.height = '100%'; overlay.style.pointerEvents = 'none'; overlay.width = glCanvas.width; overlay.height = glCanvas.height; container.appendChild(overlay); window._textOverlay = overlay; window._textOverlayCtx = overlay.getContext('2d'); window._textOverlayReady = true; }
function clearTextOverlay() { if (!window._textOverlayReady) return; var glCanvas = document.getElementById('canvas'); var overlay = window._textOverlay; if (overlay.width !== glCanvas.width || overlay.height !== glCanvas.height) { overlay.width = glCanvas.width; overlay.height = glCanvas.height; overlay.style.width = glCanvas.style.width; overlay.style.height = glCanvas.style.height; } window._textOverlayCtx.clearRect(0, 0, overlay.width, overlay.height); }
function drawTextOnOverlay(x,y,r,g,b,a,fontHeight,text) { if (!window._textOverlayReady) return; var ctx = window._textOverlayCtx; var str = UTF8ToString(text); ctx.font = 'bold ' + fontHeight + 'px Arial, sans-serif'; ctx.fillStyle = 'rgba(' + Math.round(r * 255) + ',' + Math.round(g * 255) + ',' + Math.round(b * 255) + ',' + a + ')'; ctx.fillText(str, x, y + fontHeight); }
